...
export default {
      ...
      .then((res) => {
        ...
        if (res.data.total > 1) {
          res.data.data.forEach((item) => {
            if (item.processName === 'Merge Locale News Process') {
              assignmentList.push(item)
            }
          })
        } else {
          if (res.data.data.processName === 'Merge Locale News Process') {
  ...
  methods: {
    ...
    action(item) {
      this.$router.push({ name: 'Local Daily News - Approve', query: { processId: item.processId, activityName: item.activityName } })
    }
  }
}
</script>
