...
export default {
  ...
  methods: {
    ...
    async getNewsListAsiaPacificList() {
      ...
      const getNewsListAsiaPacificList = await jogetService.list(
        "RoyalThaiCommunication",
        "list_news",
        searchData
      );
      ...
      let newsList = [];
      getNewsListAsiaPacificList.data.data.forEach(async (item, index) => {
    ...
    async getNewsListEastAsiaList() {
      ...
      const getNewsListEastAsiaList = await jogetService.list(
        "RoyalThaiCommunication",
        "list_news",
        searchData
      );
      ...
      let newsList = [];
      getNewsListEastAsiaList.data.data.forEach(async (item, index) => {
        if (item.dailyNewsProcessId === this.processId) {
          if (item.alfNodeId !== "") {
            const getFile = await this.getFileList(item.alfNodeId);
            const fileList = getFile.data.list;
