      ...
      <CCardBody>
        ...
        >
          ...
          <template #action="{item}">
            <td width="20%">
              <CButton color="info" size="sm" class="float-right" @click="action(item)"><inbox-icon size="1.5x" class="custom-class" style="padding-bottom: 2px;"></inbox-icon> ตรวจสอบข่าว</CButton>
            </td>
          </template>
        </CDataTable>
      </CCardBody>
...
export default {
  ...
  created() {
    jogetService.getMyAssignment()
      .then((res) => {
        let assignmentList = []
        if (res.data.total > 1) {
          res.data.data.forEach((item) => {
            if (item.processName === 'Merge International News Process') {
              assignmentList.push(item)
            }
          })
        } else {
          if (res.data.data.processName === 'Merge International News Process') {
  ...
  methods: {
    ...
    getDailyNewsList() {
      ...
      return jogetService.list('RoyalThaiCommunication', 'list_dailyNews', searchData)
    },
    action(item) {
      this.$router.push({ name: 'International Daily News - Approve', query: { processId: item.processId, activityName: item.activityName } })
    }
  }
}
</script>
